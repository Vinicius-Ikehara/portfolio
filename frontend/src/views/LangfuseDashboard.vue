    ...
    <main class="py-12">
      <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-12">
          ...
          <div class="w-24 h-1 mx-auto mb-6" style="background: linear-gradient(90deg, #8b5cf6, #c8703f);"></div>
          <p class="text-lg max-w-2xl mx-auto" style="color: #a39b8f;">
            ...
            <router-link to="/projects/pokedex" class="font-semibold underline underline-offset-2" style="color: #c8703f;">Pok&eacute;dex AI Agent</router-link>
            in production.
          </p>
        </div>
        ...
        <div v-if="loading" class="text-center py-12">
          ...
          <p class="mt-4 text-lg" style="color: #a39b8f;">Loading dashboard data...</p>
        </div>

        <!-- Disabled State -->
        <div v-else-if="!dashboardData.enabled" class="rounded-xl p-8 text-center" style="background-color: #262320; border: 1px solid #3a352f;">
          <i class="pi pi-eye-slash text-5xl mb-4" style="color: #6b6258;"></i>
          <p class="text-xl font-semibold mb-2" style="color: #ede7df;">Langfuse Not Connected</p>
        ...
        <div v-else-if="error" class="rounded-xl p-6 text-center" style="background-color: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.3);">
          ...
          <button @click="fetchDashboard" class="mt-4 px-4 py-2 rounded-lg text-sm font-medium" style="background-color: #c8703f; color: #ede7df;">
            Retry
          </button>
        </div>
        ...
        <div v-else>
          ...
          <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            ...
            <div class="rounded-xl p-5" style="background-color: #262320; border: 1px solid #3a352f;">
              <div class="flex items-center gap-3 mb-3">
                ...
                <span class="text-xs font-medium uppercase tracking-wider" style="color: #a39b8f;">Total Traces</span>
              </div>
              <p class="text-3xl font-bold" style="color: #ede7df;">{{ dashboardData.summary.total_traces }}</p>
            </div>
            ...
            <div class="rounded-xl p-5" style="background-color: #262320; border: 1px solid #3a352f;">
              <div class="flex items-center gap-3 mb-3">
                ...
                <span class="text-xs font-medium uppercase tracking-wider" style="color: #a39b8f;">Total Cost</span>
              </div>
              <p class="text-3xl font-bold" style="color: #ede7df;">${{ dashboardData.summary.total_cost_usd.toFixed(2) }}</p>
            </div>
            ...
            <div class="rounded-xl p-5" style="background-color: #262320; border: 1px solid #3a352f;">
              <div class="flex items-center gap-3 mb-3">
                ...
                <span class="text-xs font-medium uppercase tracking-wider" style="color: #a39b8f;">Avg Latency</span>
              </div>
              <p class="text-3xl font-bold" style="color: #ede7df;">{{ formatLatency(dashboardData.summary.avg_latency_ms) }}</p>
            </div>

            <!-- Total Observations -->
            <div class="rounded-xl p-5" style="background-color: #262320; border: 1px solid #3a352f;">
              <div class="flex items-center gap-3 mb-3">
                <div class="w-10 h-10 rounded-lg flex items-center justify-center" style="background-color: rgba(200, 112, 63, 0.2);">
                  <i class="pi pi-eye text-lg" style="color: #c8703f;"></i>
                </div>
                <span class="text-xs font-medium uppercase tracking-wider" style="color: #a39b8f;">Observations</span>
              </div>
              <p class="text-3xl font-bold" style="color: #ede7df;">{{ dashboardData.summary.total_observations }}</p>
            </div>
          </div>

          <!-- Daily Activity Bar Chart (CSS-only) -->
          <div v-if="dashboardData.daily_stats.length > 0" class="rounded-2xl p-6 md:p-8 mb-8" style="background-color: #262320; border: 1px solid #3a352f;">
            <div class="flex items-center gap-3 mb-6">
              <i class="pi pi-chart-bar text-xl" style="color: #8b5cf6;"></i>
              <h2 class="text-xl font-bold" style="color: #ede7df;">Daily Activity</h2>
            </div>
            <div class="flex items-end gap-2 h-40">
              ...
              >
                <span class="text-xs font-medium" style="color: #a39b8f;">{{ day.traces }}</span>
                <div
                ...
                ></div>
                <span class="text-[10px] mt-1" style="color: #6b6258;">{{ day.date.slice(5) }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="rounded-2xl p-6 md:p-8 mb-8" style="background-color: #262320; border: 1px solid #3a352f;">
            <div class="flex items-center gap-3 mb-6">
              ...
              <h2 class="text-xl font-bold" style="color: #ede7df;">Recent Traces</h2>
            </div>
            ...
            <div v-if="dashboardData.recent_traces.length === 0" class="text-center py-8">
              ...
              <p class="text-sm" style="color: #a39b8f;">No traces recorded yet</p>
            </div>
              ...
              <table class="w-full text-sm">
                <thead>
                  <tr style="border-bottom: 2px solid #3a352f;">
                    <th class="text-left py-3 px-3 font-semibold" style="color: #a39b8f;">Name</th>
                    <th class="text-left py-3 px-3 font-semibold" style="color: #a39b8f;">Time</th>
                    <th class="text-right py-3 px-3 font-semibold" style="color: #a39b8f;">Latency</th>
                    <th class="text-right py-3 px-3 font-semibold" style="color: #a39b8f;">Tokens</th>
                    <th class="text-right py-3 px-3 font-semibold" style="color: #a39b8f;">Cost</th>
                    <th class="text-center py-3 px-3 font-semibold" style="color: #a39b8f;">Status</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                  ...
                  >
                    <td class="py-3 px-3">
                      <span class="font-medium" style="color: #ede7df;">{{ trace.name }}</span>
                    ...
                    <td class="py-3 px-3" style="color: #a39b8f;">{{ formatTime(trace.timestamp) }}</td>
                    <td class="py-3 px-3 text-right font-mono" style="color: #a39b8f;">
        ...
        <div class="mt-12 rounded-2xl p-6 md:p-8" style="background-color: #262320; border: 1px solid #3a352f;">
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div class="text-center p-4">
              ...
              <h3 class="font-bold mb-2" style="color: #ede7df;">1. AI App</h3>
              <p class="text-sm" style="color: #a39b8f;">
            ...
            <div class="text-center p-4">
              <div class="w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center" style="background-color: rgba(200, 112, 63, 0.2);">
                <i class="pi pi-arrows-h text-2xl" style="color: #c8703f;"></i>
              </div>
              <h3 class="font-bold mb-2" style="color: #ede7df;">2. OpenTelemetry</h3>
              <p class="text-sm" style="color: #a39b8f;">
            ...
            <div class="text-center p-4">
              ...
              <h3 class="font-bold mb-2" style="color: #ede7df;">3. Langfuse</h3>
              <p class="text-sm" style="color: #a39b8f;">
            ...
            <div class="text-center p-4">
              ...
              <h3 class="font-bold mb-2" style="color: #ede7df;">4. Dashboard</h3>
              <p class="text-sm" style="color: #a39b8f;">
...
const statusStyle = (status) => {
  ...
  return 'background-color: rgba(148, 163, 184, 0.15); color: #a39b8f;'
}
